...
def train_batch_sg(model, sentences, alpha, work=None):
    ...
    """
    result = 0
    logger.debug("SENTENCES: word_vocabs %s", len(sentences))
    exit()
    for sentence in sentences:
        word_vocabs = [model.vocab[w[0]] for w in sentence if w in model.vocab and
                       model.vocab[w[0]].sample_int > model.random.rand() * 2**32]
...
def train_batch_cbow(model, sentences, alpha, work=None, neu1=None):
    ...
    logger.info("SENTENCES: word_vocabs %s", len(sentences))
    exit()
    for sentence in sentences:
        word_vocabs = [model.vocab[w[0]] for w in sentence if w in model.vocab and
                       model.vocab[w[0]].sample_int > model.random.rand() * 2**32]
...
def train_cbow_pair(model, word, input_word_indices, l1, alpha, learn_vectors=True, learn_hidden=True, weight=1.):
    ...
    if model.hs:
        ...
        ga = (weight - word.code - fa) * alpha  # vector of error gradients multiplied by the learning rate
        if learn_hidden:
...
class Word2Vec(utils.SaveLoad):
    ...
    def scan_vocab(self, sentences, progress_per=10000, trim_rule=None):
        ...
        logger.info("PROGRESS: tam_sentences = %i", len(sentences))
    ...
    def reset_weights(self):
        ...
        for i in xrange(len(self.vocab)):
            ...
            #logger.info("algo raro %s", self.index2word[i])
