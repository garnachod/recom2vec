...
def train_document_dm(model, doc_words, doctag_indexes, alpha, work=None, neu1=None,
    ...
    word_vocabs = [(model.vocab[w[0]], w[1]) for w in doc_words if w[0] in model.vocab and
                   model.vocab[w[0]].sample_int > model.random.rand() * 2**32]
    
    #for w in doc_words:
    #    logger.debug("SENTENCES: word_vocabs %s", model.vocab[w[0]])

    #word_vocabs = [model.vocab[w[0]] for w in doc_words if w[0] in model.vocab]
    
    for pos, word_peso in enumerate(word_vocabs):
        word, peso = word_peso
...
class Doc2Vec(Word2Vec):
    ...
    def scan_vocab(self, documents, progress_per=10000, trim_rule=None):
        ...
        for document_no, document in enumerate(documents):
            ...
            for word, weight in document.words:
                vocab[word] += 1
